...
from sone.models import Schedule, Light
...
def test_get_model():
    ...
    response = client.get("/sauna/%s/model" % sauna_id)
    if SOne.instance().db.exists("model_name"):
        assert response.status_code == 200
    else:
        assert response.status_code == 404

    response = client.post("/sauna/%s/model" % sauna_id, json={"sauna_id": sauna_id, 'model_name': 'MODELSTRING'})
    assert response.status_code == 200
    assert response.json() == {"sauna_id": sauna_id, 'model_name': 'MODELSTRING'}
...
def test_update_target_temperature():
    ...
    # recover to the default value
    response = client.put("/sauna/%s/temperature" % sauna_id,
                          json={"target_temperature": DEFAULT_STATUS["target_temperature"]})
...
def test_update_lights():
    ...
    for light_default in DEFAULT_STATUS['lights']:
        light = Light.deserialize(light_default)
        light_serialize = light.serialize()

    response = client.put("/sauna/%s/lights" % sauna_id, json=DEFAULT_STATUS['lights'])
    assert response.status_code == 200
    assert response.json() == SOne.instance().status
