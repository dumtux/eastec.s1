...
from .models import Heater, Status, Schedule, Program, Light
...
class SOne(Singleton):
    ...
    def set_lights(self, lights: List[Light]) -> Status:
        if len(lights) not in [1, 2, 3, 4]:
            raise HTTPException(
                status_code=422,
                detail="1 to 4 lights should be given")
        for light in lights:
            if light.name not in ['RGB_1', 'RGB_2', 'LED_MONO_1', 'LED_MONO_2']:
                raise HTTPException(
                    status_code=422,
                    detail="Light names should be 'RGB_1' and/or 'RGB_2'")
            if light.name in ['RGB_1', 'RGB_2']:
                if light.color is None:
                    raise HTTPException(
                        status_code=422,
                        detail="RGB Light Requires Color")
                self._set_rgb_light(light)
            if light.name in ['LED_MONO_1', 'LED_MONO_2']:
                if light.brightness is None:
                    raise HTTPException(
                        status_code=422,
                        detail="Mono Light Requires Brightness")
                self._set_mono_light(light)
        return self.status

    def _set_rgb_light(self, light: Light) -> None:
        if light.name not in ['RGB_1', 'RGB_2']:
            raise HTTPException(
                status_code=422,
                detail="Light names should be 'RGB_1' and/or 'RGB_2'")
        if light.name == 'RGB_1':
    ...
    def _set_mono_light(self, light: Light) -> None:
        if light.name not in ['LED_MONO_1', 'LED_MONO_2']:
            raise HTTPException(
                status_code=422,
                detail="Light names should be 'LED_MONO_1' and/or 'LED_MONO_2'")
        if light.name == 'LED_MONO_1':
            self.status.lights[2] = light
            if light.state:
                self._light_mono_1(light.brightness)
            else:
                self._light_mono_1(0)
        if light.name == 'LED_MONO_2':
            self.status.lights[3] = light
            if light.state:
                self._light_mono_2(light.brightness)
            else:
                self._light_mono_2(0)
    ...
    def _light_rgb_1(self, r: int, g: int, b: int):
        if (r not in range(256)) or (g not in range(256)) or (b not in range(256)):
            raise HTTPException(
                status_code=422,
                detail="RGB values should be in range 0-255")
    ...
    def _light_mono_1(self, brightness: int):
        if brightness not in range(101):
            raise HTTPException(
                status_code=422,
                detail="Brightness should be in range 0-100")
        if self.pwm_dict is None:
            return
        self.pwm_dict[LED_MONO_1].ChangeDutyCycle(int(brightness))

    def _light_mono_2(self, brightness: int):
        if brightness not in range(101):
            raise HTTPException(
                status_code=422,
                detail="Brightness should be in range 0-100")
        if self.pwm_dict is None:
            return
        self.pwm_dict[LED_MONO_2].ChangeDutyCycle(int(brightness))
